...
public:
	...
	LLNode(int location, int g_val, int h_val, LLNode* parent, int timestep, int num_of_conflicts = 0, bool in_openlist = false, int offset = 0) :
		location(location), g_val(g_val), h_val(h_val), parent(parent), timestep(timestep + offset),
...
public:
	...
	map< int, vector<int> > my_heuristic;  // this is the precomputed heuristic for this agent
