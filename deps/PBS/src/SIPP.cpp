...
PBSPath SIPP::findOptimalPath(const set<int>& higher_agents, const vector<PBSPath*>& paths, int agent, int start_location, int goal_location)
{
    ...
    // change agent for loc (pass idx of loc || value?)

    // generate start && add it to the OPEN list
    ...
    while (!open_list.empty())
    {
        ...
        for (int next_location : instance.getNeighbors(curr->location, curr->timestep)) // move to neighboring locations
        {
        ...
        if (curr->high_expansion == curr->high_generation &&
            reservation_table.find_safe_interval(interval, curr->location, curr->high_expansion) &&
    ...
    for (auto & old_node : ptr->second)
    {
        ...
        { // delete the old node
            if (old_node->in_openlist) // the old node has not been expanded yet
                eraseNodeFromLists(old_node); // delete it from open &&/|| focal lists
        ...
        else if(old_node->timestep < new_node->high_expansion && new_node->timestep < old_node->high_expansion)
